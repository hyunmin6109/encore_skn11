...
with open('C:\\encore_skn11\\03_web-crawling\\html_sample.html', 'r', encoding='utf-8') as f:
    html = f.read()

# BeautifulSoup = html, xml 을 parsing(쪼개서 분석)해서 데이터 추출
...
def test_find():
    ...
    # find_all : html 태그 및 속성을 dict로 조회 (전부 조회)
...
def test_selecter():
    ...
    tags = bs.select_one('.select-content')
    print(tags)
    print(type(tags))
...
def get_content():
    ...
    # 자식 요소 : secrion#section2 > h2
    # 후손 요소 : secrion#section2 li
...
def get_content2():
    # id가 section1인 section태그의 자손 h2태그의 '내용', p태그의 '내용' 출력
